 ...
 * Modes:
 *
 *   HTTP server  (recommended for Laravel – long-lived, no per-request startup):
 *     ./engine --serve               – listen on port 9000
 *     ./engine --serve --port 8080   – custom port
 *     Endpoints:
 *       GET /search?prefix=chi              → JSON results
 *       GET /search?prefix=chi&fuzzy=1      → fuzzy match
 *       GET /search?prefix=chi&top=10       → up to 10 results
 *       GET /health                         → {"status":"ok","words":N}
 *
 *   Single-shot  (simple but spawns a new process per request):
 *     ./engine "prefix"
 *     ./engine --fuzzy "prefix"
 *     ./engine --top 10 "prefix"
 *
 *   REPL / pipe:
 *     ./engine          reads lines from stdin, writes JSON per line
 *
 * Flags (all modes):
 *   --env  PATH   path to .env file (overrides SEARCH_ENGINE_ENV env var)
 *   --port N      HTTP port (default 9000, server mode only)
 *   --fuzzy / -f  fuzzy query (single-shot mode)
 *   --top N / -n  max results 1-20 (default 5)
 */
...
int main(int argc, char* argv[]) {
    ...
    // ── Load environment ──────────────────────
    // Priority: --env flag > SEARCH_ENGINE_ENV env var > ../. env (build-dir fallback)
    const char* env_path_override = std::getenv("SEARCH_ENGINE_ENV");
    std::string env_path = !env_flag.empty()    ? env_flag
                         : env_path_override     ? env_path_override
                                                 : "../.env";
    ...
    if (engine.word_count() == 0) {
        std::cerr << "[engine] No words loaded — check DB connection and that dishes table is not empty\n";
    }
